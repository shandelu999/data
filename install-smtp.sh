...
mkdir -p $opendkim_dir/$mail_domain

# 为主域名生成 DKIM 密钥
opendkim-genkey -b 2048 -d $domain -D $opendkim_dir/$domain -s mail -v
chown opendkim:opendkim $opendkim_dir/$domain/mail.private

# 为子域名生成 DKIM 密钥
opendkim-genkey -b 2048 -d $mail_domain -D $opendkim_dir/$mail_domain -s mail -v
chown opendkim:opendkim $opendkim_dir/$mail_domain/mail.private

# 配置 KeyTable
echo "mail._domainkey.$domain $domain:mail:$opendkim_dir/$domain/mail.private" | tee /etc/opendkim/KeyTable
echo "mail._domainkey.$mail_domain $mail_domain:mail:$opendkim_dir/$mail_domain/mail.private" | tee -a /etc/opendkim/KeyTable

# 配置 SigningTable
echo "*@$domain mail._domainkey.$domain" | tee /etc/opendkim/SigningTable
echo "*@$mail_domain mail._domainkey.$mail_domain" | tee -a /etc/opendkim/SigningTable
...
echo "DKIM 公钥生成完成，请将以下内容添加到 DNS："
echo "主域名 $domain 的 DKIM 公钥："
cat $opendkim_dir/$domain/mail.txt
echo "子域名 $mail_domain 的 DKIM 公钥："
cat $opendkim_dir/$mail_domain/mail.txt